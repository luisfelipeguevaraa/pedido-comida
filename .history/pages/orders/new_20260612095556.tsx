...
export default function NewOrder() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/locals').then((r) => r.json()),
    ]).then(([menus, locals]) => {
      setMenus(menus)
      setLocals(locals)
      if (locals.length > 0 && !id_local) {
        setIdLocal(locals[0].id)
      }
      setDetails((prev) =>
        prev.map((row) =>
  ...
  async function save(e: FormEvent) {
    e.preventDefault()
    if (loading) {
      setMessage('Espera a que carguen menús y locales antes de guardar')
      return
    }
    if (!id_local || !fecha || !horaPedido || !nombreCliente || !celular) {
      setMessage('Completa todos los datos del pedido')
      return
    }
    if (details.length === 0) {
      setMessage('Debes agregar al menos un detalle de pedido')
      return
    }
