  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
        <button className="btn" onClick={prevWeek}>← Anterior</button>
        <button className="btn" onClick={nextWeek}>Siguiente →</button>
      </div>
            ...
            <thead>
              <tr style={{ background: '#f5f5f5' }}>
                {['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes'].map((name, idx) => (
                  <th
                    key={name}
                    style={{ border: '2px solid #ddd', padding: '12px 8px', textAlign: 'left', fontWeight: 'bold' }}
                  >
                    <div>{name}</div>
                    <div style={{ fontSize: 12, color: '#666', marginTop: 4 }}>{formatDate(dates[idx])}</div>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {Array.from({ length: maxRows || 1 }).map((_, rowIdx) => (
                <tr key={rowIdx} style={{ borderBottom: '1px solid #eee' }}>
                  {dates.map((d) => {
                    ...
                    return (
                      <td key={key + rowIdx} style={{ border: '1px solid #eee', padding: '12px 8px' }}>
                        {val}
                      </td>
                    )
                  })}
